...
- 科学交互图解：物理接入 23 类、化学接入 23 类，共 46 类主题图解。在受力、运动、能量、波、电磁和结构反应图解之外，进一步补齐交流波形、LC 振荡、热机循环、实验拟合、物性分离和定量实验误差；可动态表达的过程提供播放、暂停、步骤推进或参数控制，并支持减少动态效果设置
...
端到端测试覆盖六学期数学、物理和化学教材结构、逐知识点讲解深度、知识族与图解族路由、科学关系不变量、关键图解交互、全部通用演示、跨学期搜索、1440/390/320px 布局，以及浅色和深色模式的 WCAG 2 A/AA 检查。46 类科学图解均有代表主题回归；新增图解会逐个切换全部模式和滑块极值，并直接测量 SVG 图元边界，动画还会在运行中切换减少动态效果，验证立即停止。
